...
import { getPaymentHistory } from "../../services/studentService";
import { IndianRupee, Hash, Calendar, Loader2, CheckCircle2, Clock, AlertCircle, History } from "lucide-react";
...
export default function MyTransactions() {
  ...
  const summaryText = useMemo(() => {
    if (!pagination.total) return "No transactions yet";
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">My Transactions</h1>
          <p className="text-grayCustom-medium mt-1 text-sm font-medium">All your subscription, course, and test series purchases.</p>
        </div>
        <div className="flex items-center gap-3 bg-dark-200 border border-dark-100 p-4 rounded-2xl shadow-lg">
          ...
          <div>
            <p className="text-[10px] font-bold text-grayCustom-medium uppercase tracking-widest leading-none">Total</p>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-dark-300/50 border-b border-dark-100">
                ...
                <th className="px-6 py-5 text-xs font-bold text-grayCustom-medium uppercase tracking-widest">Details</th>
            ...
            <tbody className="divide-y divide-dark-100">
              {loading ? (
                <tr>
                  <td colSpan="5" className="px-6 py-24 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <span className="text-grayCustom-medium text-sm font-medium">Loading transactions...</span>
                    </div>
                  </td>
                </tr>
              ) : payments.length === 0 ? (
                ...
                payments.map((payment) => {
                  ...
                  return (
                    <tr key={payment._id} className="hover:bg-dark-100/50 transition-colors">
